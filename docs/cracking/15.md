...
第 24 行的`getKeyParts()`函数将一个整数键拆分成两个整数密钥 A 和密钥 B:
...
然后我们需要从第 42 行的`getKeyParts` `()`函数中获得密钥 A 和密钥 B 的整数值，方法是将它传递给`key`。接下来，我们通过将这些值传递给`checkKeys()`函数来检查它们是否是有效的键。如果`checkKeys()`函数没有导致程序退出，那么键是有效的，第 43 行之后的`encryptMessage()`函数中的剩余代码可以继续执行。
