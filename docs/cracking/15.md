...
要加密文本，我们需要计算加密信件的索引。第 49 行将这个`symbolIndex`乘以`keyA`，并将`keyB`加到乘积上。然后，它根据符号集的大小修改结果，用表达式`len(SYMBOLS)`表示。由`len(SYMBOLS)`进行的调制通过确保计算出的指数总是在`0`和不包括`len(SYMBOLS)`之间来处理绕回。得到的数字将是加密字符在`SYMBOLS`中的索引，它被连接到`ciphertext`中字符串的末尾。
